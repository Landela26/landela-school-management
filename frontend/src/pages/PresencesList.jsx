...
export default function PresencesList() {
  ...
  useEffect(() => { setPage(1) }, [date, classId, statut])
  ...
  }, [page, date, classId, statut])

  useEffect(() => { load() }, [load])

  const hasFilters = Boolean(date || classId || statut)
  ...
  const resetFilters = () => { setDate(''); setClassId(''); setStatut('') }
    ...
    <div className="mx-auto max-w-6xl p-6 md:p-8">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select id="f-classe" value={classId} onChange={(e) => setClassId(e.target.value)} className={filterInput}>
            <option value="">Toutes les classes</option>
            {classesList.map((c) => (
              <option key={c.id_classe} value={c.id_classe}>{c.nom_classe}</option>
            ))}
          </select>
        </div>
        <div className="flex flex-col gap-1.5">
