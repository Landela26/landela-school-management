...
import {
  AlertCircle, ChevronLeft, ChevronRight, PencilLine, Search, UserPlus, Users, X,
} from 'lucide-react'
...
function initials(student) {
  return ((student?.nom?.charAt(0) || '') + (student?.prenom?.charAt(0) || '')).toUpperCase() || '?'
}
...
function SexeBadge({ sexe }) {
  ...
  return <span className={`inline-flex rounded-md px-2.5 py-1 text-xs font-medium ${item.className}`}>{item.label}</span>
}
...
function SkeletonRows() {
  return Array.from({ length: 6 }).map((_, i) => (
    <tr key={i} className="border-t border-slate-100">
      ...
      <td className="px-5 py-4"><div className="ml-auto h-9 w-9 animate-pulse rounded-md bg-slate-100" /></td>
    </tr>
  ))
}
...
export default function StudentsList() {
  ...
  useEffect(() => { setPage(1) }, [debounced, sexe])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res?.success) throw new Error(res?.message || 'Réponse inattendue du serveur.')
      setStudents(res.data || [])
      setPagination(res.pagination || null)
    } catch (err) {
      console.error('Erreur liste élèves :', err)
      setError(err.response?.data?.message || err.message || 'Impossible de charger la liste des élèves.')
      setStudents([]); setPagination(null)
    } finally {
      setLoading(false)
    }
  }, [page, debounced, sexe])
  ...
  const resetFilters = () => { setSearch(''); setSexe('') }
    ...
    <div className="mx-auto max-w-6xl p-6 md:p-8">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold tracking-tight text-slate-900 md:text-3xl">Élèves</h1>
        </div>
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Rechercher par nom…" aria-label="Rechercher un élève par nom"
        ...
        <div className="flex items-center gap-1 rounded-md border border-slate-200 bg-white p-1">
          {SEXE_FILTERS.map((opt) => (
            <button key={opt.value || 'all'} type="button" onClick={() => setSexe(opt.value)}
              className={`rounded px-3 py-1.5 text-sm font-medium transition ${
                sexe === opt.value ? 'bg-navy text-white' : 'text-slate-500 hover:bg-slate-100'}`}>
              {opt.label}
            </button>
          ))}
        </div>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        {error ? (
          ...
          <div className="flex min-h-[320px] flex-col items-center justify-center px-6 text-center">
            ...
            <h2 className="mb-1 text-base font-semibold text-slate-900">{hasFilters ? 'Aucun résultat' : 'Aucun élève'}</h2>
            <p className="mb-6 max-w-sm text-sm leading-6 text-slate-500">
              {hasFilters ? 'Aucun élève ne correspond à votre recherche. Essayez d’autres critères.'
            ...
            {hasFilters ? (
              ...
              <button onClick={() => navigate('/eleves/nouveau')}
                className="inline-flex items-center gap-2 rounded-md bg-navy px-4 py-2.5 text-sm font-medium text-white transition hover:bg-navy-dark">
                <UserPlus size={17} />Ajouter un élève
              </button>
            )}
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] border-collapse text-left">
              ...
              <tbody>
                {loading ? <SkeletonRows /> : students.map((s) => (
                  <tr key={s.id_eleve} className="border-t border-slate-100 transition hover:bg-slate-50/70">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold text-slate-900">{s.nom} {s.postnom}</p>
                          <p className="truncate text-xs text-slate-500">{s.prenom}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-sm tabular-nums text-slate-500">{s.matricule || '—'}</td>
                    <td className="px-5 py-4"><SexeBadge sexe={s.sexe} /></td>
                    <td className="px-5 py-4 text-sm tabular-nums text-slate-500">{formatDate(s.date_naissance)}</td>
                    <td className="px-5 py-4"><StatutBadge statut={s.statut} /></td>
                    <td className="px-5 py-4">
                      <div className="flex justify-end">
                        <button type="button" onClick={() => navigate(`/eleves/${s.id_eleve}/modifier`)}
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-500">
              {loading ? 'Chargement…' : (
                ...
                {' '}sur <span className="font-medium text-slate-900 tabular-nums">{total}</span> élèves</>
              )}
            </p>
