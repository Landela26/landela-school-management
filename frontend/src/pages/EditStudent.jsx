...
export default function EditStudent() {
  ...
  useEffect(() => {
    const loadStudent = async () => {
      try {
        ...
        setError(err.response?.data?.message || err.message || 'Impossible de charger les informations de l’élève.')
      } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSuccess(response?.message || 'Les informations de l’élève ont été mises à jour.')
    } catch (err) {
      ...
      if (err.response?.status === 422) {
        ...
        setError('Impossible de modifier l’élève. Vérifiez votre connexion et réessayez.')
      }
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500">Chargement des informations de l’élève...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="mx-auto max-w-3xl p-6 md:p-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="font-display text-2xl font-bold tracking-tight text-slate-900">Modifier un élève</h1>
        </div>
      </div>
